$(function(){
    $("button[name='update']").click(function(){
        ...
        var selectPro=getSelect(professions,stuPro,"stuProfession");
        $("tr:eq(0)>td:eq(0)").html('<strong>姓名：</strong><input type="text" name="stuName" value="'+name+'"/>');
        $("tr:eq(1)>td:eq(0)").html('<strong>年龄：</strong><input type="text" name="stuAge" value="'+age+'"/>');
        $("tr:eq(2)>td:eq(0)").html('<strong>性别：</strong>'+radioSex);
        $("tr:eq(3)>td:eq(0)").html('<strong>学号：</strong><input type="text" name="stuNum" value="'+stuNum+'"/>');
        $("tr:eq(4)>td:eq(0)").html('<strong>学院：</strong>'+selectCollege);
        $("tr:eq(5)>td:eq(0)").html('<strong>专业：</strong>'+selectPro);
        $("tr:eq(6)>td:eq(0)").html('<strong>籍贯：</strong><input type="text" name="stuAddress" value="'+address+'"/>');
        $("tr:eq(7)>td:eq(0)").html('<strong>邮箱：</strong><input type="text" name="stuEmail" value="'+email+'"/>');
        $("tr:eq(8)>td:eq(0)").html('<strong>电话：</strong><input type="text" name="stuPhone" value="'+phone+'"/>');
        $("tr:eq(9)>td:eq(0)").html('<strong>  QQ：</strong><input type="text" name="stuQQ" value="'+QQ+'"/>');
...
var getSelect=function(options,value,name){
   	 	var strOption="<select name='"+name+"'>";
 ...
 var getRadio=function(array,value){
	 ...
	 for(var i=0;i<array.length;i++){
	 	 if(value==array[i]) 
	 	 {
	 	 	strRadio+="<input type='radio' name='stuSex' value='"+array[i]+"' checked />"+array[i];
	 	 }else{
	 	 	strRadio+="<input type='radio' name='stuSex' value='"+array[i]+"'  />"+array[i];
	 	 }
...
*/
 var getTextArea= function(value){
     var strTextArea="<textarea cols='60' rows='5' name='stuRecommend'>"+value+"</textarea>";
     return strTextArea;
 }
